...
class Product {
  ...
  async getOne(req, res) {
    try {
      ...
      if (product) {
        ...
        return res.status(HTTP_STATUS.BAD_REQUEST).send(`failed to recieve product`);
      }
    } catch (error) {
  ...
  async add(req, res) {
    try {
      ...
      // }
      const { name, author, category, description, releaseDate, pages, price, stock } = req.body;
      ...
      await user.save();
      return res.status(HTTP_STATUS.CREATED).send(success("Book Added Successfully", user));
    } catch (error) {
      console.log(error);
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).send(failure("Internal server error"));
    }
  ...
  async delete(req, res) {
    try {
      ...
      return res.status(HTTP_STATUS.ACCEPTED).send(success("Product deleted successfully", deletedItem));
    } catch (error) {
      console.error(error);
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).send(failure("Internal server error"));
    }
  ...
  async update(req, res) {
    try {
      ...
      return res.status(HTTP_STATUS.ACCEPTED).send(success("Product updated successfully", updatedProduct));
    } catch (error) {
